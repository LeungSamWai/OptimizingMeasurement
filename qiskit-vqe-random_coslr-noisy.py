...
def simulator(theta, shots, history):
    ...
    ratio = np.random.dirichlet(np.ones(3), size=1)[0]
    
    
    shots1 = round(shots * ratio[0])
    if shots1 < 1:
        shots1 = 1
        
    shots2 = round(shots * ratio[1])
    if shots2 < 1:
        shots2 = 1
        
    shots3 = round(shots * ratio[2])
    if shots3 < 1:
        shots3 = 1
    print(shots1, shots2, shots3)
...
if __name__ == '__main__':
    ...
    ## shot for each
    # ADAM optimizer
    result = adam(lambda x: simulator(x, shots=args.shots, history=record_history), theta_test, iters=500, eps=0.02)

    np.savez('qiskit_adam2d/Random-noisyvqe_adam_shots{}_init_n1-eps0p02-iter500-coslr-trial{}.npz'.format(args.shots, args.trial),

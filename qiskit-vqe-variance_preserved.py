...
def adam(fun, x0, iters=100, options=None, beta1=0.9, beta2=0.999, epsilon=1e-8, eps=0.02):
    ...
    while nit < maxiter:
        ...
        fval, grad = func_and_gradient(x_opt, fun, eps)
        nfev += 2
        njev += 1
...
def simulator(theta, shots, std_shots, history):
    ...
    newshots = (shots - std_shots*3)*sum(ratio_for_theta)**2/3/sum([v**2 for v in ratio_for_theta])
...
if __name__ == '__main__':
    ...
    # ADAM optimizer
    result = adam(lambda x: simulator(x, shots=args.shots, std_shots=args.std_shots, history=record_history), theta_test, iters=500, eps=0.005)
    # print(result)

    np.savez('qiskit_adam1d_zero_initialization/Ratio-std-onfly-newtotal-noisyvqe_adam_shots{}_init_-2-eps0p005-iter500-coslr-stdshot{}-trial{}.npz'.format(args.shots, args.std_shots, args.trial),
